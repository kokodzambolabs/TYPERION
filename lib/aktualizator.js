...
export async function aktualizujWynikiCore(supabase) {
  ...
  for (const mecz of mecze) {
    ...
    // Wynik 90 MINUT (regulaminowy czas) - dla full_time_*, home/away_score
    // i punktacji pucharowej. Football-Data v4: gdy mecz miał dogrywkę/karne,
    // score.fullTime zawiera wynik PO dogrywce (lub łączny z karnymi), a wynik
    // 90 min siedzi w score.regularTime. Gdy mecz skończył się w regulaminowym,
    // regularTime nie ma - wtedy fullTime JEST wynikiem 90 min.
    const reg = apiMecz.score?.regularTime;
    const wynik90Home = reg?.home ?? fullTimeHome;
    const wynik90Away = reg?.away ?? fullTimeAway;

    if (nowyStatus === 'finished' && wynik90Home != null && wynik90Away != null) {
      // full_time_* i home/away_score to spójnie wynik 90 min. Dla pucharowych
      // karne/dogrywka nie wchodzą do wyniku - rozstrzygnięcie idzie przez
      // winner_team_id. Dla grupowych regularTime i tak nie ma, więc to fullTime.
      aktualizacja.full_time_home_score = wynik90Home;
      aktualizacja.full_time_away_score = wynik90Away;
      aktualizacja.home_score = wynik90Home;
      aktualizacja.away_score = wynik90Away;
